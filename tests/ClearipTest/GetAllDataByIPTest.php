<?php
use IPtrace\IPInfoApi;

class SampleTest extends PHPUnit_Framework_TestCase
{
    public function testGetAllDataByIP()
    {
        $IPTraceClientStub = $this->getMockBuilder('IPtrace\Client')->disableOriginalConstructor()->getMock();

        $fakeHttpClient = $this->getMockBuilder('\IPtrace\HttpClient')
        ...
        $IPTraceClientStub->IPInfoApi = new IPInfoApi($fakeHttpClient);
        $response = $IPTraceClientStub->IPInfoApi->GetAllDataByIP('192.168.1.1');
